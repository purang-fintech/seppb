...
CREATE TABLE `sepp_user_setting` (
  ...
  `table_show_border` tinyint(1) DEFAULT 0 COMMENT '表格是否展示边框',
  `table_page_size` int(3) DEFAULT 20 COMMENT '表格每页最多展示记录数',
  `created_date` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '创建日期',
  `updated_date` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp() COMMENT '更新日期',
  PRIMARY KEY (`id`),
...
CREATE TABLE `sepp_warning_notify` (
  ...
  `from` int(10) NOT NULL COMMENT '发送人',
  `to` int(10) NOT NULL COMMENT '接收人',
  `send_time` timestamp NULL DEFAULT NULL COMMENT '计划发送时间',
  `sent_time` timestamp NULL DEFAULT NULL COMMENT '发送时间',
  `is_sent` varchar(1) NOT NULL COMMENT '是否已发送',
  `send_success` varchar(1) NOT NULL COMMENT '是否已送达',
  `is_read` varchar(1) NOT NULL COMMENT '是否已读',
  `updated_date` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp() COMMENT '更新日期',
  `created_date` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '创建日期',
  PRIMARY KEY (`id`),
  KEY `INDEX_TO_SEND` (`send_gateway`,`from`,`to`,`is_sent`),
  KEY `INDEX_MESSAGE` (`warning_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
